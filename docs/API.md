...
| POST | `/brokers/{broker_id}/connection-check` | **Read-only probe** (Phase 94, D113). Logs in and reads the balance through the adapter's `get_account_state`; places no order. Always 200 while the probe RAN — a venue's refusal comes back as `reachable: false` with the venue's own words, because that is a finding and not a server error. Reports `credential_source` (`stored`/`environment`), the real `cash` figure (null on any failure, never 0), position SYMBOLS only, and `account` — the PUBLIC credential fields, so the answer names WHICH account it reached (for IG, DEMO or LIVE). A secret is never in any part of this response. `admin:manage`. |
| POST | `/brokers/providers/{provider}/connection-check` | **Read-only probe against the ENVIRONMENT**, with no broker row (Phase 95, D114). Same contract and same shared code path as the per-broker probe; consults env credentials only, because a stored credential is addressed by broker row. 404 for an unknown provider. `admin:manage`. |
