...
1. **Candle patterns → intraday signals.** `candles.py` produces patterns;
   wiring them as entries on the intraday engine needs that engine to be
   persisted/exposed first (see `docs/AUDIT.md` gap B). Then a
   candle-pattern setup joins the `SETUPS` registry.
