...
| Market Data | Vendor routing, no-fabrication sentinels | Vendor APIs | Normalized snapshots | Longbridge (`longport` SDK) | Phase 5, 12, 14 (done) — Longbridge wired (D015), reachable via `GET /market-data/{symbol}/quote`; connected to trade submission for an omitted `estimated_price` (D017) |
