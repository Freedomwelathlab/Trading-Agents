...
- **Phase 28 — frontend v3: historical performance chart, admin listing UI
  + its backend endpoints, session expiry UX (2026-08-29, D030/D031).**
  ...
  (D030).** `GET /admin/users`, `GET /admin/roles`, and
  ...
  (3) **Session refresh/expiry UX (D031).** New `GET /auth/session`
...
and Phase 24/D026, now further reduced by Phase 28/D030/D031 (the
...
Phase 28 (D030/D031) adds 18, bringing the backend total to **226**: admin
